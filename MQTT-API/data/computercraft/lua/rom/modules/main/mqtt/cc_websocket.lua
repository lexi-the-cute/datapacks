...
function cc_websocket.receive(conn, size)
    if (not conn.received_value) or conn.received_value == '' then
        local value, is_binary = conn.sock.receive(conn.timeout)
        if not value then return nil, "timeout" end

        conn.received_value = value
    end

    local retval = string_sub(conn.received_value, 1, size)
    conn.received_value = string_sub(conn.received_value, size + 1)
    ...
    -- print(string.format("read %d bytes, recv length remaining: %d, retval length: %d", size, conn.received_value:len(),
    --     retval:len()))
